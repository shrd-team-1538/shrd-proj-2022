            ...
            <form @submit.prevent class="flex-form">
                <div class="form-wrapper grow-3">
                    ...
                    <div class="form-wrapper">
                        ...
                        <RouterLink to="/signin" class="have-account">У меня есть аккаунт</RouterLink>
                    </div>
                </div>
                <button type="submit" class="submit" @click="signup">Создать аккаунт</button>
            </form>
        </div>
    </div>
</template>
...
const signup = async () => {
    ...
    console.log(result);
    if (result.conflict) return alert("Имя пользователя уже используется");
    if (!result.ok) return alert("Произошла ошибка");
